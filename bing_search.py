...
def chrome_bing(keyword, target):
    ...
    try:
        ...
        if control.Exists(5, 1):
            control.Click()
            control.SendKeys("https://www.bing.com")
            control.SendKeys("{Enter}")
        else:
            return False

        logger.info(f"[4/6] 페이지 로드 확인")
        while browser.Name != "Bing - Chrome":
            time.sleep(0.1)

        browser = auto.PaneControl(Name="Bing - Chrome")
        ...
        control = browser.EditControl(Name="Enter your search term")
        if control.Exists(5, 1):
        ...
        for page_no in range(1, 10 + 1):
            ...
            control = browser.TextControl(Name=target)
...
if __name__== '__main__':
    chrome_bing('로또', 'http://lottobingo.kr')