...
def chrome_naver_blog(keyword, target):
    ...
    try:
        ...
        if control.Exists(5, 1):
            control.Click()
            control.SendKeys("https://section.blog.naver.com/Search/Post.nhn?pageNo=1&rangeType=ALL&orderBy=sim&keyword=")
            control.SendKeys("{Enter}")
        else:
            return False

        logger.info(f"[4/6] 페이지 로드 확인")
        while browser.Name != "네이버 블로그 - Chrome":
            time.sleep(0.1)
        browser = auto.PaneControl(Name="네이버 블로그 - Chrome")
        ...
        control = browser.EditControl(Name="검색어를 입력하고 버튼을 누르세요")
        if control.Exists(5, 1):
        ...
        for page_no in range(1, 10 + 1):
            ...
            if page_no == 1:
                ...
                control = browser.HyperlinkControl(Name=f'{page_no}페이지')
                if control.Exists(5, 1):
...
if __name__ == '__main__':
    chrome_naver_blog('로또블루', '로또블루 알고하는 재미!')
